...
>(
  async (currency, address, accountId, fromBlock) => {
    ...
    return ops
      .map((event, index) =>
        etherscanERC20EventToOperation(accountId, event, index)
      )
      .filter(Boolean) as Operation[];
  },
  (currency, address, accountId, fromBlock) => accountId + fromBlock,
    ...
    return Object.values(opsByHash)
      .map((events) =>
        events.map((event, index) =>
          etherscanERC721EventToOperation(accountId, event, index)
        )
      )
      .flat();
  },
...
>(
  async (currency, address, accountId, fromBlock) => {
    ...
    return Object.values(opsByHash)
      .map((events) =>
        events.map((event, index) =>
          etherscanERC1155EventToOperation(accountId, event, index)
        )
      )
      .flat();
  },
